...
### Polling and Query-State Ownership

Treat polling, deferred props, and infinite-scroll loads as concurrent Inertia visits. A background response owns only the props it refreshes; it never owns a tab, view, filter, sort, cursor, or other foreground query state.

- Set `preserveUrl: true` on background refreshes that must not navigate.
- Specify `only`, `reset` or merge intent, and polling concurrency mode explicitly.
- Keep query-derived props out of background `only` lists unless the refresh intentionally owns them.
- Cancel or reject stale async requests when a newer same-path query navigation starts. Prevent unrelated background work from starting while a foreground visit is in flight, while allowing deferred props belonging to that new page to load.
- Update client-only query state with Inertia client visits such as `router.replace(...)`, not `window.history.replaceState`, so Inertia's page URL and browser history stay aligned.
- Consider polling, deferred props, and `InfiniteScroll` together; they share Inertia's async request stream and can finish out of order.

For a regression, delay a background response, change query-backed UI state, release the response, and assert the URL, selected control, page props, and rendered data still describe the same scope.
...
- Assuming `preserveUrl` prevents stale props from being applied; it protects URL ownership, not response ordering
- Letting direct History API writes diverge from Inertia's in-memory page URL
